...
const getTiers = () => getBasePricingTiers({
  industryName: "PMU business",
})
  {
    name: "Launch",
    tagline: "Professional online presence for your PMU business.",
    setupPrice: "$249",
    monthlyPrice: "$99",
    popular: false,
    included: [
      "Custom PMU website design",
      "Service-focused landing pages (microblading, lip blush, ombre)",
      "Booking request & contact forms",
      "Lead management CRM system",
      "Email notifications for new client inquiries",
  ...
  {
    name: "Growth",
    tagline: "Convert more beauty leads into loyal PMU clients automatically.",
    setupPrice: "$549",
    monthlyPrice: "$149",
    popular: true,
    included: [
      ...
      "Visual lead pipeline (Inquiry → Consultation → Booked)",
      "Post-appointment Google review automation",
      "Unified inbox (SMS, email, web leads)",
      "Mobile app for studio management",
      "Client tagging & service segmentation",
  ...
  {
    name: "Accelerator",
    tagline: "Scale your PMU business with advanced automation.",
    setupPrice: "$999",
    monthlyPrice: "$299",
    popular: false,
    included: [
      "Everything in Growth",
      "Automated PMU service pricing calculator",
      "Smart follow-up sequences by service type",
      "Dormant client win-back campaigns",
      "Touch-up & maintenance reminder automation",
...
const features = [
  {
    icon: MessageSquare,
    title: "Never Miss a PMU Client",
    description: "Instant automated responses to every microblading and lip blush inquiry, even when you're with clients.",
  },
  {
    icon: Star,
    title: "Build Your Beauty Reputation",
    description: "Automated Google review requests sent after every PMU appointment to grow your 5-star ratings.",
  },
  {
    icon: Users,
    title: "Manage All Your Clients",
    description: "Track all PMU clients, service history, touch-ups, and before/after photos in one organized CRM.",
  },
  {
    icon: Zap,
    title: "Automated Lead Nurturing",
    description: "SMS and email sequences that convert PMU inquiries into booked appointments.",
  },
  {
    icon: Clock,
    title: "Reclaim 10+ Hours Weekly",
    description: "Stop manually following up on consultations. Let automation handle the admin work.",
  },
  {
    icon: BarChart3,
    title: "Track Your Growth",
    description: "See which services (microblading, ombre brows, lip blush) bring the most revenue.",
  },
]
...
const comparisonData = {
  without: [
    "Missed calls during PMU sessions",
    "Consultations forgotten by clients",
  ...
  with: [
    ...
    "Automated consultation follow-up sequences",
    "Reviews requested after every appointment",
    "All conversations in one place",
    "Full visibility into your pipeline",
    "System works while you create beauty",
  ],
}

const faqData = [
  {
    q: "How quickly can my PMU website go live?",
    a: "Most PMU and permanent makeup websites launch within 3-7 business days. We handle the entire build — you provide your services, portfolio, and branding.",
  },
  {
    q: "Do I need technical skills to manage this system?",
    a: "Not at all. Everything is built for you and managed from your phone. We provide training and ongoing support for your PMU business.",
  },
  {
    q: "Can I upgrade my plan as my PMU business grows?",
    a: "Absolutely. Upgrade or downgrade anytime with no penalties. Perfect for solo PMU artists and beauty studios.",
  },
  {
    q: "What if I already have a PMU website?",
    a: "We can integrate our CRM and automation with your existing site, or build a new modern PMU website. We'll recommend the best option during your consultation.",
  },
]
...
// Interactive CRM Mockup Component with 3D Mouse Tracking - PMU Version
function InteractiveCRMMockup() {
  ...
  useEffect(() => {
    ...
    const leadsInterval = setInterval(() => {
      currentLeads += 1
      setNewLeads(Math.min(currentLeads, 26))
      if (currentLeads >= 26) clearInterval(leadsInterval)
    }, 100)
    
    let currentConv = 0
    const convInterval = setInterval(() => {
      currentConv += 1
      setConversions(Math.min(currentConv, 19))
      if (currentConv >= 19) clearInterval(convInterval)
    }, 150)
    
    let currentRev = 0
    const revInterval = setInterval(() => {
      currentRev += 280
      setRevenue(Math.min(currentRev, 6720))
      if (currentRev >= 6720) clearInterval(revInterval)
    }, 80)
    
    let currentReviews = 0
    const reviewsInterval = setInterval(() => {
      currentReviews += 1
      setReviews(Math.min(currentReviews, 92))
      if (currentReviews >= 92) clearInterval(reviewsInterval)
    }, 60)
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          style={{
            background: `radial-gradient(600px circle at ${glowX}% ${glowY}%, rgba(236,72,153,0.15), rgba(219,39,119,0.1), transparent 40%)`,
            opacity: isHovered ? 1 : 0,
          }}
        />

        {/* Browser frame */}
        <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-zinc-800 to-zinc-900 p-1 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_0_60px_rgba(236,72,153,0.15)] transition-shadow duration-300 hover:shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6),0_0_80px_rgba(236,72,153,0.2)]">
          {/* Browser header */}
          <div className="flex items-center gap-2 px-4 py-3 bg-zinc-900/80 rounded-t-xl border-b border-white/5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="text-[10px] text-pink-400">Live</span>
            </div>
          </div>
              ...
              {activeTab === 'leads' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-[10px] text-white/50">PMU CRM</p>
                      <p className="text-sm font-semibold text-white">Client Pipeline</p>
                    </div>
                    <motion.button
                  ...
                  {/* Lead cards */}
                  {[
                    { name: 'Emily R.', service: 'Microblading', status: 'hot', time: 'Just now' },
                    { name: 'Sophia M.', service: 'Lip Blush', status: 'warm', time: '12 min ago' },
                    { name: 'Isabella K.', service: 'Ombre Brows', status: 'new', time: '18 min ago' },
                  ].map((lead, idx) => (
                    <motion.div
                      key={idx}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold ${
                          lead.status === 'hot' ? 'bg-pink-500/20 text-pink-400' :
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-white/40">7 unread</span>
                      <div className="w-5 h-5 rounded-full bg-pink-500/20 flex items-center justify-center">
                        <span className="text-[10px] font-bold text-pink-400">7</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Message cards */}
                  {[
                    { name: 'Emily R.', msg: 'Interested in microblading...', channel: 'SMS', unread: true, time: '5m' },
                    { name: 'Sophia M.', msg: 'Can I book lip blush tomorrow?', channel: 'Email', unread: true, time: '18m' },
                    { name: 'Isabella K.', msg: 'Ombre brows consultation?', channel: 'Web', unread: true, time: '1h' },
                    { name: 'Olivia T.', msg: 'My brows look amazing, thanks!', channel: 'SMS', unread: false, time: '4h' },
                  ].map((msg, idx) => (
                    <motion.div
                      key={idx}
                      className={`rounded-xl p-3 border cursor-pointer transition-all ${
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-500/30 to-fuchsia-500/30 flex items-center justify-center text-[10px] font-bold text-white">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[8px] px-1.5 py-0.5 rounded ${
                              msg.channel === 'SMS' ? 'bg-pink-500/20 text-pink-400' :
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    ...
                    <div className="rounded-xl bg-white/5 p-2.5 border border-white/10">
                      ...
                      <p className="text-lg font-bold text-pink-400">+22</p>
                    </div>
                  </div>

                  {/* Review cards */}
                  {[
                    { name: 'Jessica L.', rating: 5, text: 'Best microblading artist! Perfect brows!', time: '2 days ago' },
                    { name: 'Ashley W.', rating: 5, text: 'Love my lip blush. So natural looking!', time: '1 week ago' },
                    { name: 'Megan D.', rating: 5, text: 'Professional, gentle, and talented!', time: '2 weeks ago' },
                  ].map((review, idx) => (
                    <motion.div
                      key={idx}
      ...
      >
        <div className="flex items-center gap-2">
          <motion.div 
            className="w-8 h-8 rounded-full bg-pink-500/10 flex items-center justify-center"
            animate={{ scale: [1, 1.2, 1] }}
      ...
      >
        ...
        <p className="text-[9px] text-white/80 max-w-[120px]">&ldquo;Best PMU artist ever!&rdquo;</p>
        <p className="text-[8px] text-white/50 mt-1">— Beauty Studio</p>
      </motion.div>
...
function AnimatedCounter({ value, label }: { value: string; label: string }) {
  return (
    ...
    >
      <div className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-pink-500 via-fuchsia-500 to-pink-600 bg-clip-text text-transparent">
        {value}
      </div>
...
export default function PMUArtistsPage() {
  return (
    <>
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 overflow-hidden">
        {/* Background effects */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(236,72,153,0.08),transparent_50%)] pointer-events-none" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,rgba(219,39,119,0.08),transparent_50%)] pointer-events-none" />
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center justify-items-center lg:justify-items-start">
            ...
            >
              ...
              >
                ...
                <span className="text-sm font-medium text-foreground-secondary">For PMU & Permanent Makeup Artists</span>
              </motion.div>
              ...
              <p className="text-lg sm:text-xl text-foreground-secondary mb-8 leading-relaxed">
                Complete PMU business growth system: capture microblading leads, automate appointment follow-ups, collect 5-star reviews, and manage all your clients — so you can focus on creating perfect brows, lips, and eyeliner, not chasing consultations.
              </p>
              
              <div className="flex flex-col sm:flex-row gap-3 mb-8 justify-center lg:justify-start">
                <Link
                  href="#pricing"
                  className="inline-flex items-center justify-center gap-2 h-12 px-7 bg-gradient-to-r from-pink-600 to-fuchsia-600 text-white font-semibold rounded-xl transition-all duration-200 hover:brightness-110 shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_4px_12px_rgba(236,72,153,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] active:scale-[0.98]"
                >
                  View PMU Pricing
      ...
      <section className="py-20 relative">
        <Container>
          <SectionHeading
            ...
            title="Running a PMU Business Without vs. With Automation"
          ...
          <div className="grid md:grid-cols-2 gap-6 mt-12 max-w-4xl mx-auto">
            ...
            >
              ...
              <div className="relative rounded-2xl border border-pink-500/30 bg-gradient-to-br from-pink-500/5 to-fuchsia-500/5 p-6">
                <div className="flex items-center gap-2 mb-4">
      ...
      <section className="py-20 relative">
        ...
        <Container>
          <SectionHeading
            overline="PMU FEATURES"
            title="Everything Your PMU Business Needs"
            highlightWord="Needs"
            subtitle="Stop juggling spreadsheets and missed calls. One system for microblading consultations, lip blush bookings, ombre brow leads, and client management."
          />
            ...
            {features.map((feature, index) => (
              ...
              >
                ...
                <div className="relative rounded-2xl border border-white/10 bg-[linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] p-6 h-full transition-all duration-300 hover:border-white/20">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-pink-500/20 to-fuchsia-500/20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
      ...
      <section id="pricing" className="py-20 relative">
        <Container>
          <SectionHeading
            overline="PMU PRICING"
            title="Transparent Pricing for PMU Artists"
            highlightWord="Pricing"
            subtitle="No hidden fees. No surprises. Perfect for microblading artists, PMU specialists, and beauty studios."
          />
          ...
          <div className="mt-12">
            <PricingTiers tiers={getTiers()} accentColor="primary" />
              ...
              >
                ...
                <div className={`relative h-full rounded-2xl border ${tier.popular ? 'border-pink-500/50' : 'border-white/10'} bg-[linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] overflow-hidden transition-all duration-300 hover:border-white/20`}>
                  {/* Popular badge */}
                  {tier.popular && (
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-pink-500 via-fuchsia-500 to-pink-600" />
                  )}
                  ...
                  <div className="p-6 sm:p-8 flex flex-col h-full">
                    ...
                    <div className="mb-6">
                      {tier.popular && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold text-white bg-gradient-to-r from-pink-500 via-fuchsia-500 to-pink-600 mb-3">
                    ...
                    <Link
                      ...
                      className={`w-full inline-flex items-center justify-center gap-2 h-11 px-5 font-semibold rounded-xl transition-all duration-200 mt-auto active:scale-[0.98] ${
                        tier.popular 
                          ? 'bg-gradient-to-r from-pink-600 to-fuchsia-600 text-white hover:brightness-110 shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_4px_12px_rgba(236,72,153,0.4),inset_0_1px_0_rgba(255,255,255,0.1)]'
      ...
      <section className="py-20 relative">
        ...
        <Container>
          <SectionHeading
            ...
            title="Launch Your PMU System in Days"
            highlightWord="Days"
          />
            ...
            {[
              {
                ...
                text: "15-minute consultation to discuss your PMU services, portfolio, service areas, and branding.",
                icon: Calendar,
              },
              {
                num: "02",
                title: "We Build Your System",
                text: "Custom website, CRM, and automation workflows built specifically for your PMU business. Ready in 3-7 days.",
                icon: Zap,
              },
              {
                num: "03",
                title: "Start Booking Clients",
                text: "Go live and start converting consultations into bookings. We monitor performance and provide ongoing support.",
                icon: BarChart3,
              },
            ].map((step, index) => (
              <motion.div
              ...
              >
                ...
                <div className="relative rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(236,72,153,0.08),transparent_38%),radial-gradient(circle_at_80%_0%,rgba(219,39,119,0.08),transparent_40%),linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] p-6 h-full transition-all duration-300 hover:border-white/20 text-center">
                  {/* Step number badge */}
                  <span className="inline-flex items-center justify-center text-3xl font-bold bg-gradient-to-br from-pink-500/40 to-fuchsia-500/40 bg-clip-text text-transparent mb-4 group-hover:from-pink-500 group-hover:to-fuchsia-500 transition-all duration-300">
                    {step.num}
                  </span>
                  
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-pink-500/20 to-fuchsia-500/20 flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300 border border-pink-500/20">
                ...
                {index < 2 && (
                  <div className="hidden md:flex absolute top-1/2 -right-3 -translate-y-1/2 w-6 h-6 items-center justify-center">
                    <ChevronRight className="w-5 h-5 text-pink-500/40" />
                  </div>
                )}
              </motion.div>
            ))}
      ...
      <section className="py-20 relative">
        <Container>
          <SectionHeading
            overline="PMU FAQ"
            title="Common Questions from PMU Artists"
      ...
      {/* CTA Section */}
      <section className="py-20 relative">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(236,72,153,0.08),transparent_50%)] pointer-events-none" />
        ...
        <Container>
          ...
          >
            ...
            <div className="relative rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(236,72,153,0.12),transparent_38%),radial-gradient(circle_at_80%_80%,rgba(219,39,119,0.12),transparent_40%),linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] p-8 sm:p-12 text-center">
              ...
              <p className="text-foreground-secondary max-w-2xl mx-auto mb-8 leading-relaxed">
                Book a free 15-minute consultation. We&apos;ll discuss your PMU services, portfolio, and show you how to automate your beauty business growth.
              </p>
              <Link
                href="/schedule-a-call"
                className="inline-flex items-center justify-center gap-2 h-12 px-7 bg-gradient-to-r from-pink-600 to-fuchsia-600 text-white font-semibold rounded-xl transition-all duration-200 hover:brightness-110 shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_4px_12px_rgba(236,72,153,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] active:scale-[0.98]"
              >
                <span className="hidden sm:inline">Schedule Your Free PMU Consultation</span>
                <span className="inline sm:hidden">Schedule Call</span>
                <ArrowRight className="w-5 h-5" />
              </Link>
              <p className="text-xs text-foreground-secondary mt-4">
                No commitment required • Perfect for microblading artists and PMU specialists
              </p>
            </div>
          </motion.div>
        </Container>
        ...
        <Container>
          <div className="text-center">
            <p className="text-sm text-foreground-secondary">
              Need a custom solution for your PMU business?{" "}
              <Link href="/contact-us" className="text-pink-500 hover:text-pink-400 transition-colors underline">
                Contact us
              </Link>{" "}
              and we&apos;ll create a tailored package for your beauty services.
            </p>
          </div>
        </Container>
      </section>
