...
const getTiers = () => getBasePricingTiers({
  industryName: "pool cleaning business",
})
  {
    name: "Launch",
  ...
  },
  {
    ...
    included: [
      ...
      "Automated review requests after services",
    ],
    notIncluded: [
...
const features = [
  {
    ...
    description: "Automatic responses to inquiries within seconds, even when you're on a job site.",
  },
  {
    icon: Star,
  ...
  },
  {
    ...
    description: "All your clients, conversations, and service history in one organized place.",
  },
  {
    icon: Zap,
    title: "Instant Follow-Up",
    description: "SMS and email sequences that nurture leads while you focus on pools.",
  },
  {
    icon: Clock,
...
const comparisonData = {
  without: [
    "Missed calls while servicing pools",
  ...
  with: [
    ...
    "System works while you service",
  ],
}
...
function InteractiveCRMMockup() {
  ...
  useEffect(() => {
    ...
    const leadsInterval = setInterval(() => {
      currentLeads += 1
      setNewLeads(Math.min(currentLeads, 15))
      if (currentLeads >= 15) clearInterval(leadsInterval)
    }, 100)
    
    let currentConv = 0
    const convInterval = setInterval(() => {
      currentConv += 1
      setConversions(Math.min(currentConv, 11))
      if (currentConv >= 11) clearInterval(convInterval)
    }, 150)
    
    let currentRev = 0
    const revInterval = setInterval(() => {
      currentRev += 180
      setRevenue(Math.min(currentRev, 3240))
      if (currentRev >= 3240) clearInterval(revInterval)
    }, 80)
    
    let currentReviews = 0
    const reviewsInterval = setInterval(() => {
      currentReviews += 1
      setReviews(Math.min(currentReviews, 52))
      if (currentReviews >= 52) clearInterval(reviewsInterval)
    }, 60)
  ...
  return (
    ...
    >
        ...
        <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-zinc-800 to-zinc-900 p-1 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_0_60px_rgba(123,99,255,0.15)] transition-shadow duration-300 hover:shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6),0_0_80px_rgba(255,106,85,0.2)]">
          ...
          <div className="bg-[#0f0b0e] rounded-b-xl min-h-[340px] overflow-hidden">
            ...
            >
              ...
              {activeTab === 'leads' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-[10px] text-white/50">Pool Service CRM</p>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <motion.div 
                      ...
                      animate={{ scale: newLeads === 15 ? [1, 1.02, 1] : 1 }}
                      transition={{ duration: 0.3 }}
                    >
                  ...
                  {/* Lead cards */}
                  {[
                    { name: 'David K.', service: 'Weekly Maintenance', status: 'hot', time: 'Just now' },
                    { name: 'Amy R.', service: 'Pool Repair', status: 'warm', time: '5 min ago' },
                    { name: 'Tom W.', service: 'Green Pool Cleanup', status: 'new', time: '12 min ago' },
                  ].map((lead, idx) => (
                    <motion.div
                      key={idx}
              ...
              {/* Inbox View - using same structure */}
              {activeTab === 'inbox' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-white/40">4 unread</span>
                  ...
                  {/* Message cards */}
                  {[
                    { name: 'David K.', msg: 'Need pool maintenance quote...', channel: 'SMS', unread: true, time: '2m' },
                    { name: 'Amy R.', msg: 'Can you service next week?', channel: 'Email', unread: true, time: '15m' },
                    { name: 'Tom W.', msg: 'Pool is green, help!', channel: 'Web', unread: true, time: '1h' },
                    { name: 'Sarah B.', msg: 'Great service, thank you!', channel: 'SMS', unread: false, time: '3h' },
                  ].map((msg, idx) => (
                    <motion.div
                      key={idx}
                      className={`rounded-xl p-3 border cursor-pointer transition-all ${
              ...
              {activeTab === 'reviews' && (
                <div className="space-y-3">
                  ...
                  {/* Review cards */}
                  {[
                    { name: 'Jennifer B.', rating: 5, text: 'Best pool service ever! Crystal clear water.', time: '2 days ago' },
                    { name: 'Robert M.', rating: 5, text: 'Very professional. Highly recommend!', time: '1 week ago' },
                    { name: 'Amanda S.', rating: 5, text: 'Fixed my green pool in no time. Amazing!', time: '2 weeks ago' },
                  ].map((review, idx) => (
                    <motion.div
                      key={idx}
      ...
      >
        ...
        <p className="text-[9px] text-white/80 max-w-[120px]">&ldquo;Best pool service ever!&rdquo;</p>
        <p className="text-[8px] text-white/50 mt-1">— Crystal Clear Pools</p>
      </motion.div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center justify-items-center lg:justify-items-start">
            ...
            >
              ...
              >
                ...
                <span className="text-sm font-medium text-foreground-secondary">For Pool Service Businesses</span>
              </motion.div>
              ...
              <p className="text-lg sm:text-xl text-foreground-secondary mb-8 leading-relaxed">
                A complete system that captures pool service leads, follows up automatically, collects reviews, and keeps everything organized — so you can focus on pools, not paperwork.
              </p>
