...
const getTiers = () => getBasePricingTiers({
...
})
  {
    ...
    popular: false,
    included: [
      "Custom Roofing website design",
      "Service-focused landing pages (roofing, framing, builds)",
      "Estimate request & contact forms",
      "Lead management CRM system",
      "Email notifications for new job inquiries",
  ...
  {
    name: "Growth",
    tagline: "Convert more Roofing leads into signed contracts automatically.",
    setupPrice: "$549",
    monthlyPrice: "$149",
    popular: true,
    included: [
      ...
      "Visual lead pipeline (Estimate → Scheduled → Active)",
      "Post-job Google review automation",
      "Unified inbox (SMS, email, web leads)",
      "Mobile app for job site management",
      "Client tagging & job segmentation",
      "Unlimited website content updates",
      "24/7 system monitoring",
      "Live chat widget for instant inquiries",
      "job milestone reminder automation",
    ],
    notIncluded: [
  ...
  {
    name: "Accelerator",
    tagline: "Scale your Roofing business with advanced automation.",
    setupPrice: "$999",
    monthlyPrice: "$299",
    popular: false,
    included: [
      "Everything in Growth",
      "Automated Roofing estimate calculator",
      "Smart follow-up sequences by job type",
      "Dormant client win-back campaigns",
      "job type upsell automation",
      "Priority 24/7 support",
      "Monthly performance & ROI reports",
      "Online job booking portal",
      "Advanced review filtering & responses",
      "Multi-job client management",
    ],
    notIncluded: [],
...
const features = [
  {
    icon: MessageSquare,
    title: "Never Miss a Roofing Lead",
    description: "Instant automated responses to every roofing and building inquiry, even when you're on the job site.",
  },
  {
    icon: Star,
    title: "Build Your Reputation Fast",
    description: "Automated Google review requests sent after every completed job to grow your 5-star ratings.",
  },
  {
    icon: Users,
    title: "Manage All Your jobs",
    description: "Track all Roofing clients, job history, and timelines in one organized CRM.",
  },
  {
    icon: Zap,
    title: "Automated Lead Nurturing",
    description: "SMS and email sequences that convert Roofing estimates into signed contracts.",
  },
  {
    icon: Clock,
    title: "Reclaim 10+ Hours Weekly",
    description: "Stop manually following up on job bids. Let automation handle the admin work.",
  },
  {
    icon: BarChart3,
    title: "Track Your Growth",
    description: "See which services (roofing, new builds, renovations) bring the most revenue.",
  },
]
...
const comparisonData = {
  without: [
    "Missed calls during Roofing jobs",
...
const faqData = [
  {
    q: "How quickly can my Roofing website go live?",
    a: "Most Roofing and roofing websites launch within 3-7 business days. We handle the entire build — you provide your services, service areas, and branding.",
  },
  {
    q: "Do I need technical skills to manage this system?",
    a: "Not at all. Everything is built for you and managed from your phone. We provide training and ongoing support for your Roofing business.",
  },
  {
    q: "Can I upgrade my plan as my Roofing business grows?",
    a: "Absolutely. Upgrade or downgrade anytime with no penalties. Perfect for seasonal Roofing businesses.",
  },
  {
    q: "What if I already have a Roofing website?",
    a: "We can integrate our CRM and automation with your existing site, or build a new modern Roofing website. We'll recommend the best option during your consultation.",
  },
]
...
// Interactive CRM Mockup Component with 3D Mouse Tracking - Roofing Version
function InteractiveCRMMockup() {
  ...
  useEffect(() => {
    ...
    const leadsInterval = setInterval(() => {
      currentLeads += 1
      setNewLeads(Math.min(currentLeads, 22))
      if (currentLeads >= 22) clearInterval(leadsInterval)
    }, 100)
    
    let currentConv = 0
    const convInterval = setInterval(() => {
      currentConv += 1
      setConversions(Math.min(currentConv, 16))
      if (currentConv >= 16) clearInterval(convInterval)
    }, 150)
    
    let currentRev = 0
    const revInterval = setInterval(() => {
      currentRev += 850
      setRevenue(Math.min(currentRev, 18700))
      if (currentRev >= 18700) clearInterval(revInterval)
    }, 80)
    
    let currentReviews = 0
    const reviewsInterval = setInterval(() => {
      currentReviews += 1
      setReviews(Math.min(currentReviews, 78))
      if (currentReviews >= 78) clearInterval(reviewsInterval)
    }, 60)
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          style={{
            background: `radial-gradient(600px circle at ${glowX}% ${glowY}%, rgba(239,68,68,0.15), rgba(220,38,38,0.1), transparent 40%)`,
            opacity: isHovered ? 1 : 0,
          }}
        />

        {/* Browser frame */}
        <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-zinc-800 to-zinc-900 p-1 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_0_60px_rgba(239,68,68,0.15)] transition-shadow duration-300 hover:shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6),0_0_80px_rgba(239,68,68,0.2)]">
          {/* Browser header */}
            ...
            >
              ...
              {activeTab === 'leads' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-[10px] text-white/50">Roofing CRM</p>
                  ...
                  {/* Lead cards */}
                  {[
                    { name: 'David R.', service: 'Roof Replacement', status: 'hot', time: 'Just now' },
                    { name: 'Patricia S.', service: 'Roof Repair', status: 'warm', time: '8 min ago' },
                    { name: 'Thomas W.', service: 'Gutter Installation', status: 'new', time: '15 min ago' },
                  ].map((lead, idx) => (
                    <motion.div
                      key={idx}
              ...
              {activeTab === 'inbox' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center">
                        <span className="text-[10px] font-bold text-red-400">6</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Message cards */}
                  {[
                    { name: 'David R.', msg: 'Need Roof Replacement quote...', channel: 'SMS', unread: true, time: '3m' },
                    { name: 'Patricia S.', msg: 'When can you start the addition?', channel: 'Email', unread: true, time: '10m' },
                    { name: 'Thomas W.', msg: 'Gutter Installation estimate?', channel: 'Web', unread: true, time: '45m' },
                    { name: 'Karen B.', msg: 'Deck looks amazing, thank you!', channel: 'SMS', unread: false, time: '2h' },
                  ].map((msg, idx) => (
                    <motion.div
                      key={idx}
                      className={`rounded-xl p-3 border cursor-pointer transition-all ${
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-500/30 to-slate-500/30 flex items-center justify-center text-[10px] font-bold text-white">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[8px] px-1.5 py-0.5 rounded ${
                              msg.channel === 'SMS' ? 'bg-red-500/20 text-red-400' :
              ...
              {activeTab === 'reviews' && (
                <div className="space-y-3">
                  ...
                  {/* Review cards */}
                  {[
                    { name: 'Tom B.', rating: 5, text: 'Best roofer! Roof Replacement is perfect.', time: '3 days ago' },
                    { name: 'Rachel P.', rating: 5, text: 'Professional work on our Roof Repair.', time: '1 week ago' },
                    { name: 'Mike D.', rating: 5, text: 'On time, on budget. Highly recommend!', time: '2 weeks ago' },
                  ].map((review, idx) => (
                    <motion.div
                      key={idx}
      ...
      >
        ...
        <p className="text-[9px] text-white/80 max-w-[120px]">&ldquo;Best roofer ever!&rdquo;</p>
        <p className="text-[8px] text-white/50 mt-1">— BuildRight Co.</p>
      </motion.div>
...
function AnimatedCounter({ value, label }: { value: string; label: string }) {
  return (
    ...
    >
      <div className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-red-500 via-slate-500 to-red-600 bg-clip-text text-transparent">
        {value}
      </div>
  ...
  return (
    <>
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        {/* Background effects */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(239,68,68,0.08),transparent_50%)] pointer-events-none" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,rgba(220,38,38,0.08),transparent_50%)] pointer-events-none" />
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center justify-items-center lg:justify-items-start">
            ...
            >
              ...
              >
                ...
                <span className="text-sm font-medium text-foreground-secondary">For Roofing & roofing Businesses</span>
              </motion.div>
              
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
                Grow Your Roofing Business{" "}
                <span className="bg-gradient-to-r from-red-500 via-slate-500 to-red-600 bg-clip-text text-transparent">
                  On Autopilot
                </span>
              </h1>
              
              <p className="text-lg sm:text-xl text-foreground-secondary mb-8 leading-relaxed">
                Complete Roofing business growth system: capture roofing leads, automate job follow-ups, collect 5-star reviews, and manage all your builds — so you can focus on framing, foundations, and finishing jobs, not chasing estimates.
              </p>
              
              <div className="flex flex-col sm:flex-row gap-3 mb-8 justify-center lg:justify-start">
                <Link
                  href="#pricing"
                  className="inline-flex items-center justify-center gap-2 h-12 px-7 bg-gradient-to-r from-red-600 to-slate-600 text-white font-semibold rounded-xl transition-all duration-200 hover:brightness-110 shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_4px_12px_rgba(239,68,68,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] active:scale-[0.98]"
                >
                  View Roofing Pricing
      ...
      <section className="py-20 relative">
        <Container>
          <SectionHeading
            ...
            title="Running a Roofing Business Without vs. With Automation"
          ...
          <div className="grid md:grid-cols-2 gap-6 mt-12 max-w-4xl mx-auto">
            ...
            >
              ...
              <div className="relative rounded-2xl border border-red-500/30 bg-gradient-to-br from-red-500/5 to-slate-500/5 p-6">
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-8 h-8 rounded-full bg-red-500/10 flex items-center justify-center">
      ...
      <section className="py-20 relative">
        ...
        <Container>
          <SectionHeading
            overline="Roofing FEATURES"
            title="Everything Your Roofing Business Needs"
            highlightWord="Needs"
            subtitle="Stop juggling spreadsheets and missed calls. One system for roofing quotes, new build estimates, renovation leads, and job management."
          />
            ...
            {features.map((feature, index) => (
              ...
              >
                ...
                <div className="relative rounded-2xl border border-white/10 bg-[linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] p-6 h-full transition-all duration-300 hover:border-white/20">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500/20 to-slate-500/20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
      ...
      <section id="pricing" className="py-20 relative">
        <Container>
          <SectionHeading
            overline="Roofing PRICING"
            title="Transparent Pricing for roofers"
          ...
          <div className="mt-12">
            <PricingTiers tiers={getTiers()} accentColor="red" />
              ...
              >
                ...
                <div className={`relative h-full rounded-2xl border ${tier.popular ? 'border-red-500/50' : 'border-white/10'} bg-[linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] overflow-hidden transition-all duration-300 hover:border-white/20`}>
                  {/* Popular badge */}
                  {tier.popular && (
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-red-500 via-slate-500 to-red-600" />
                  )}
                  ...
                  <div className="p-6 sm:p-8 flex flex-col h-full">
                    ...
                    <div className="mb-6">
                      {tier.popular && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold text-white bg-gradient-to-r from-red-500 via-slate-500 to-red-600 mb-3">
                    ...
                    <Link
                      ...
                      className={`w-full inline-flex items-center justify-center gap-2 h-11 px-5 font-semibold rounded-xl transition-all duration-200 mt-auto active:scale-[0.98] ${
                        tier.popular 
                          ? 'bg-gradient-to-r from-red-600 to-slate-600 text-white hover:brightness-110 shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_4px_12px_rgba(239,68,68,0.4),inset_0_1px_0_rgba(255,255,255,0.1)]'
      ...
      <section className="py-20 relative">
        ...
        <Container>
          <SectionHeading
            ...
            title="Launch Your Roofing System in Days"
            highlightWord="Days"
          />
            ...
            {[
              {
                ...
                text: "15-minute consultation to discuss your Roofing services, job types, service areas, and branding.",
                icon: Calendar,
              },
              {
                num: "02",
                title: "We Build Your System",
                text: "Custom website, CRM, and automation workflows built specifically for your Roofing business. Ready in 3-7 days.",
                icon: Zap,
              },
              {
                num: "03",
                title: "Start Capturing job Leads",
                text: "Go live and start converting roofing quotes into contracts. We monitor performance and provide ongoing support.",
                icon: BarChart3,
              },
            ].map((step, index) => (
              <motion.div
              ...
              >
                ...
                <div className="relative rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(239,68,68,0.08),transparent_38%),radial-gradient(circle_at_80%_0%,rgba(220,38,38,0.08),transparent_40%),linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] p-6 h-full transition-all duration-300 hover:border-white/20 text-center">
                  {/* Step number badge */}
                  <span className="inline-flex items-center justify-center text-3xl font-bold bg-gradient-to-br from-red-500/40 to-slate-500/40 bg-clip-text text-transparent mb-4 group-hover:from-red-500 group-hover:to-slate-500 transition-all duration-300">
                    {step.num}
                  </span>
                  
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-red-500/20 to-slate-500/20 flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300 border border-red-500/20">
      ...
      <section className="py-20 relative">
        <Container>
          <SectionHeading
            overline="Roofing FAQ"
            title="Common Questions from roofers"
      ...
      {/* CTA Section */}
      <section className="py-20 relative">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(239,68,68,0.08),transparent_50%)] pointer-events-none" />
        ...
        <Container>
          ...
          >
            ...
            <div className="relative rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(239,68,68,0.12),transparent_38%),radial-gradient(circle_at_80%_80%,rgba(220,38,38,0.12),transparent_40%),linear-gradient(160deg,#181116_0%,#0f0b0e_100%)] p-8 sm:p-12 text-center">
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground mb-4">
                Ready to Grow Your{" "}
                <span className="bg-gradient-to-r from-red-500 to-slate-500 bg-clip-text text-transparent">
                  Roofing Business?
                </span>
              </h2>
              <p className="text-foreground-secondary max-w-2xl mx-auto mb-8 leading-relaxed">
                Book a free 15-minute consultation. We&apos;ll discuss your Roofing services, job types, and show you how to automate your business growth.
              </p>
              <Link
                href="/schedule-a-call"
                className="inline-flex items-center justify-center gap-2 h-12 px-7 bg-gradient-to-r from-red-600 to-slate-600 text-white font-semibold rounded-xl transition-all duration-200 hover:brightness-110 shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_4px_12px_rgba(239,68,68,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] active:scale-[0.98]"
              >
                <span className="hidden sm:inline">Schedule Your Free Roofing Consultation</span>
                <span className="inline sm:hidden">Schedule Call</span>
                <ArrowRight className="w-5 h-5" />
              </Link>
              <p className="text-xs text-foreground-secondary mt-4">
                No commitment required • Perfect for roofers, remodelers, and builders
              </p>
            </div>
          </motion.div>
        </Container>
        ...
        <Container>
          <div className="text-center">
            <p className="text-sm text-foreground-secondary">
              Need a custom solution for your Roofing business?{" "}
              <Link href="/contact-us" className="text-red-500 hover:text-red-400 transition-colors underline">
                Contact us
              </Link>{" "}
              and we&apos;ll create a tailored package for your contracting services.
            </p>
          </div>
        </Container>
      </section>
